...
public class CurrentJapaneseUserInputSelectionService(IJishoRepository jishoRepository)
    : ICurrentJapaneseUserInputSelectionService
{
    ...
    public async Task<UpdateSelectionResult> UpdateSelectionAsync(string preprocessedInput)
    {
        var allResults = await jishoRepository.GetResultAsync(preprocessedInput);
        if (allResults == null || !allResults.Any()) // TODO move out of this service
        {
            ClearUserInputResults();

            return HandleSearchError(allResults);
        }
        ...
        return new UpdateSelectionResult(true);
    }

    private UpdateSelectionResult HandleSearchError(IEnumerable<JishoDatum>? allResults)
    {
        if (allResults == null) // Application could not retrieve information from Jisho
            return new UpdateSelectionResult(false, "Error", "Information could not be retrieved!");
        else
            return new UpdateSelectionResult(false, "Information", "No results were found!");
    }

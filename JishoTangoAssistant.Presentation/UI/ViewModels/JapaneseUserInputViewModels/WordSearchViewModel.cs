...
public partial class WordSearchViewModel : JishoTangoAssistantViewModelBase, IRecipient<PreEnteredInputsUpdatedMessage>
{
    ...
    private async Task ProcessInput()
    {
        ...
        var updateSelectionResult = await currentSelectionService.UpdateSelectionAsync(Input);
        if (!updateSelectionResult.IsSuccessful)
        {
            await MessageBoxUtil.CreateAndShowAsync(updateSelectionResult.Title ?? string.Empty, updateSelectionResult.Message ?? string.Empty, MessageBoxButtons.Ok);
        }

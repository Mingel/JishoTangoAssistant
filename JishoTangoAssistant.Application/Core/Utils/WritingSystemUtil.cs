...
public static partial class WritingSystemUtil
{
    ...
    public static bool OnlyContainsRomaji(string text) => RomajiRegex().Matches(text).FirstOrDefault()?.Length == text.Length;

    public static bool OnlyContainsHiragana(string text) => HiraganaRegex().Matches(text).FirstOrDefault()?.Length == text.Length;

    public static bool OnlyContainsKatakana(string text) => KatakanaRegex().Matches(text).FirstOrDefault()?.Length == text.Length;

    public static bool OnlyContainsKanji(string text) => KanjiRegex().Matches(text).FirstOrDefault()?.Length == text.Length;

    public static bool OnlyContainsKana(string text) => OnlyContainsHiragana(text) || OnlyContainsKatakana(text);

    public static bool ContainsKanji(string text) => KanjiRegex().Matches(text).Count > 0;

    public static string FilterKanji(string text) => KanjiRegex().Match(text).Value;
